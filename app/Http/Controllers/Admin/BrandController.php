...
class BrandController extends Controller
{
    ...
    public function index(){
        ...
        /*删除*/
        // session(['user'=>null]);
        // echo request()->session()->pull('username');//先获取再删除sm
        ...
        $data=unserialize(Redis::get('data_'.$page.'_'.$word.'_'.$desc));
        ...
        // cache(['data_'.$page.'_'.$word.'_'.$desc=>$data,],30);
        $data=serialize($data);
        Redis::set('data_'.$page.'_'.$word.'_'.$desc,$data);
        }
        
        // $logs = DB::getQueryLog();
        
        $query=request()->all(); //搜索
        return view('brand.index',['data'=>$data,'query'=>$query]); //搜索后加的query
    }
